...
def ray_controller_learning_evaluation_actor_factory(
        config: EAConfig
    ...
    class RayControllerLearningEvaluationActor(EvaluationActor):
        ...
        def config(
                self
                ) -> RayControllerLearningEvaluatorConfig:
            return super().config
        ...
        def _learn_controller(
                self,
                robot: Robot
                ) -> None:
            environment = self._create_environment(robot)
            robot.controller.set_environment(environment)
            robot.controller.learn(
                total_timesteps=self.config.total_timesteps, callback=self._callback
                )
            environment.close()
        ...
        def evaluate(
            ...
            evaluation_result = EvaluationResult(
                genome=genome, fitness=post_learn_reward, info={
                        "episode_failures": {"physics": 0, "validity": 0},
                        "rewards": {"pre_learn": pre_learn_reward, "post_learn": post_learn_reward}}
                )
