...
class DistributedWandbInitialisationEvaluationCallback(EvaluationCallback):
    ...
    def _initialise_wandb(
        ...
        assert isinstance(
            self.ea_config.logger_config, WandBLoggerConfig
            ), "WandbEvaluationCallback requires the Logger to be" \
        ...
        self._wandb_run = wandb.init(
            project=project_name, group=group, tags=tags, name=name, resume="allow", id=name, sync_tensorboard=True
            )
