...
from erpy.framework.evaluator import EvaluationActor, Evaluator, EvaluatorConfig
...
class DistributedEvaluatorConfig(EvaluatorConfig, ABC):
    ...
    def actor_factory(
            self
            ) -> Callable[[DistributedEvaluatorConfig], Type[EvaluationActor]]:
        raise NotImplementedError


@dataclass
class RayEvaluatorConfig(EvaluatorConfig, ABC):
    num_workers: int
    num_cores_per_worker: int
    ...
    def evaluate(
        ...
        for genome in tqdm(
            ...
            self.pool.submit(
                    lambda
                        worker,
                        genome: worker.evaluate.remote(genome=genome), genome
                    )
